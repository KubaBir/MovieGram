...
from core.models import FriendRequest, UserProfile
...
class FriendSystemTests(TestCase):
    ...
    def test_accepting_a_friend_request(self):
        ...
        self.assertIn(self.first_user, UserProfile.objects.filter(
            user=self.second_user).get().friends.all())
    ...
    def test_declining_a_friend_request(self):
        ...
        self.assertNotIn(self.first_user, UserProfile.objects.filter(
            user=self.second_user).get().friends.all())
    ...
    def test_sending_the_same_request_twice(self):
        ...
        self.assertEqual(
            res2.data, {'Status': 'You have sent this invite already'})
        res3 = self.client1.get(MY_INVS)
    ...
    def test_sending_friend_request_when_friend_already_in_your_list(self):
        ...
        self.assertEqual(
            res3.data, {"Status": "The User is already in your friends "})
    ...
    def test_sending_friend_request_to_yourself(self):
        ...
        res1 = self.client.post(SEND_INV_URL, payload)
        self.assertEqual(
            res1.data, {"Status": "You cant send a user invite to yourself "})
