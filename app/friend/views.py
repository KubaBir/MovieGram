...
class FriendRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.request.user.is_superuser == True:
            ...
            queryset= models.FriendRequest.objects.filter(receiver=self.request.user).all()
    ...
    def sending_inv(self,request):
        ...
        if models.FriendRequest.objects.filter(sender = self.request.user.id, receiver=serializer.validated_data['receiver']).exists():
