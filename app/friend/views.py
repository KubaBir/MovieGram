...
class FriendRequestViewSet( mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == 'invitations_sent_by_me':
            queryset = models.FriendRequest.objects.filter(sender = self.request.user).all()
        else:
    ...
    def invitations_sent_by_me(self, request):
        ...
        if unsend:
            ...
            friend_reqs = models.FriendRequest.objects.filter(id__in=unsend_ids)
        ...
        print(models.FriendRequest.objects.filter(sender = self.request.user)[0].sender)
        return Response(serializer.data, status = status.HTTP_200_OK )
